...
def get_a_meme(model, img_torch, img_pil, vocab, T=1., beam_size=7, top_k=50) -> Image:
    vocabulary = vocab
    delimiter=' '
    max_len = 32
...
def memeify_image(img, top='', bottom='', font_path=FONT_PATH):
...
def get_maximal_font(img, text, font_size=64, text_width=0.94, font_path=FONT_PATH):
...
def _get_initial_font(img, texts, max_chars=20, font_path=FONT_PATH):
...
def _get_final_font(img, text_lines, font_path=FONT_PATH):
